...
def state_prep_test(p_1q, p_2q, p_meas, p_1q_z):
    for states in ['0', '1', '+', '-']:
...
def Figure3(shots = 10000,
    ...
    probs_a1 = Figure3_experiment([D1, D3], 1, labels_2q, Shots=shots, p_1q=p_1q, p_2q=p_2q, p_meas=p_meas, p_1q_z=p_1q_z)

    labels_4q = ["".join(x) for x in itertools.product("01", repeat=4)] # 0000 ~ 1111
    probs_a2 = Figure3_experiment([D1, D2, D3, D4], 0, labels_4q , Shots=shots, p_1q=p_1q, p_2q=p_2q, p_meas=p_meas, p_1q_z=p_1q_z)

    probs_a3 = Figure3_experiment([D2, D4], 2, labels_2q, Shots=shots, p_1q=p_1q, p_2q=p_2q, p_meas=p_meas, p_1q_z=p_1q_z)
...
def plot_figure5(p_1q, p_2q, p_meas, p_1q_z, shots=20000, rounds=10):
    ...
    print(f"=== Experiment Start (Shots={shots}, Rounds={rounds}) ===")

    for idx, state in enumerate(state_labels):
        # state preparation
        builder = state_prep(state, p_1q, p_2q, p_meas, p_1q_z)
        # 추가 라운드 반복
        for r in range(rounds):
            builder.measure_all(is_first_round=False)
        # 2. 샘플링
        circuit = builder.get_circuit()
        det_sampler = circuit.compile_detector_sampler()
        raw_result = det_sampler.sample(shots=shots)

        # 3. Reshape (자동 계산된 actual_rounds 사용)
